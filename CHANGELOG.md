...
### Fixed
- **Toolbar icon color was delayed after profile activation**. Activating a profile from the popup did not immediately repaint the toolbar icon; it stayed gray until the user interacted with the address bar. Root cause: while the popup has focus, `chrome.tabs.query({currentWindow:true})` returned no result because the popup itself is a `windowType:'popup'`. Fixed by resolving the active tab through `chrome.windows.getLastFocused({windowTypes:['normal']})`.
